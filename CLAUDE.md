...
- Fase 4C: personal-trainer-alphen-aan-den-rijn, sportschool-zoeterwoude en beste-personal-trainer-leiderdorp op `ds2`: inline design-styles (68-79 per pagina) naar `.art-*`-klassen in een lokale `<style>` (tokens); donkere pagina (body zwart), lime banen vervangen door graphite/soft-black, kaartgrid met 14px gap en 10px radius, Google Fonts-link toegevoegd.
- Fase 4D: bedankt, intake/bedankt, bedrijfsfitness en veteranenkorting op `ds2` (lokale `<style>` naar tokens/16px). bedankt en intake/bedankt waren onleesbaar (witte achtergrond met lichte tekst) en zijn nu donker (`.thank-wrap` zwart). Let op: `fiscfree`-logo op bedrijfsfitness is wit en dus laag zichtbaar op de lichte kaart (bestaand assetprobleem, niet aangepast).
- Fase 4E: /trainingsadvies/ op `ds2` (was ongecommit/ongelinkt; nu in de repo). Scripts en formulier ongewijzigd; door JS gegenereerde inline stijlen (knoppen 12-13px, `strong` #fff) worden via `!important` CSS-overrides op `.ds2 .calc-result`/`.advies-mail` geharmoniseerd.
- Fase 4F: /calorie-calculator/ op `ds2` (styling only; berekening en script ongewijzigd). Lime kcal-vlak vervangen door graphite kaart met lime getal; macrokleuren oranje (#ff8c2e) en blauw (#2ec4ff) bewust behouden als datavisualisatie.
- Fase 4G: /intake/ (5-stappen Netlify-formulier, noindex) op `ds2`; alleen CSS. Formulier, hidden fields, action (/intake/bedankt/), scripts en tracking byte-identiek. Inputs/keuzekaarten 16px en 48px hoog.
- Fase 4H: /gratis-intake/ op `ds2` (geen `<form>`: Virtuagym-widget, WhatsApp/mail/bel-kanalen). Scripts, tracking (fbq Lead), links en widget-tag identiek. WhatsApp-groen (#25D366) bewust behouden. Alle seo.css-pagina's staan hiermee op ds2.
- Fase 5A: /over-fit-up/ en /privacybeleid/ (eigen CSS) op `ds2`: legacy-variabelen naar tokens, oude lime/zwart weg, 16px-tekst, 10px kaarten. Over-fit-up: teamkaarten op mobiel horizontaal (foto links), footer-tekst was donker-op-donker en is licht gemaakt. Privacybeleid bewust rustig gehouden (alleen kleur/typografie/spacing).
- Fase 5B: /trainingsaanbod/, personal-training, sgpt-small-group-personal-training en 24-7-fitness (eigen CSS) op `ds2`: :root naar tokens, oude lime/zwart weg, 16px-informatietekst, 10px kaarten. Grote lime vlakken (12-weken-band, uitgelichte tarief-kaart `.tarief-card.hot`) zijn graphite met lime rand/knop; footer-tekst licht gemaakt. `/trainingsaanbod/small-group-pt/` is alleen een redirect (meta refresh + JS + 301 in _redirects) en is ongewijzigd.
- Fase 5C: /online-coaching/ (eigen CSS, echt formulier) op `ds2`: tokens, 16px-tekst, inputs 16px/48px, formulier/scripts/tracking byte-identiek. Homepage gecontroleerd: alleen radii (`.wa-float` pill en `.lite`-foto) en kleine tekst (accordion 16px, verlopen wa-notice 14px) gelijkgetrokken; overige `#000`/`#fff` zijn mask-image en WhatsApp-merkknop. /contact/ is redirect (meta refresh + 301 naar /gratis-intake/), ongewijzigd. Alle echte pagina's zijn nu gemigreerd.
- Final QA (sep 2026): alle content-/toolpagina's op ds2 of tokens. `ds2` bewust BEHOUDEN in seo.css: `_seo_engine.js` en `.github/autoblog.js` (AutoSEO-workflow, wekelijks) genereren nog pagina's met `<body>` zonder ds2 en oude kleuren; pas na aanpassing van de generators kan de legacy-laag (seo.css regels 1-265) en het `.ds2`-prefix weg. `ad-landing.html` (noindex advertentiepagina) en `/404.html` waren niet gemigreerd: 404 is opgeschoond, ad-landing staat nog op oude kleuren (apart traject). Opgeschoond: ongebruikte `.try-*`, `.photo-placeholder`, zwart-gradients in online-coaching/personal-training.
- Ad-landing: /ad-landing.html (noindex advertentiepagina met Make-webhook-formulier) op tokens/ds2-klasse: alleen CSS en inline stijlen; formulier, scripts, Meta Pixel (PageView, InitiateCheckout, Lead met eventID) en webhook byte-identiek. Voor/na-balken graphite i.p.v. lime; JS-foutmelding `#form-error` via CSS-override leesbaar (rood 16px).
- Generator-alignment (sep 2026): `.github/autoblog.js` (weekblog, template: body `ds2`, fonts + seo.css?v=20260927, `BLOG_CSS`-tokenblok met `.art-*`/`.blog-*`, geen inline lime/zwart) en `_seo_engine.js` (`<body class="ds2">`, seo.css?v=20260927) genereren nu direct in het huidige designsysteem. `.github/autoseo.js` genereert geen sitepagina's (alleen e-mailrapport via Brevo + title-fixes); niet aangepast. `_seo_engine.js` is een tijdelijke build-engine (PAYN wordt extern gedefinieerd, niet in de workflow gebruikt). Nog niet gedaan: legacy-laag in seo.css (regels 1-265) en `.ds2`-prefix verwijderen; kan pas na een Netlify-preview-check.
